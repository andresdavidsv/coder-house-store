...
socket.on('msg-from-server', (data) => {
  if (data.status !== 'error' && data.chatMessages.length > 0 ) {
    let messagesPrint = data.chatMessages.map(chatMessages =>
      {return `<li> <strong class="text-primary">${chatMessages.user}</strong> <span class="text-danger">${chatMessages.timestamp}</span>: <span class="text-success fst-italic">${chatMessages.messsage}</span></li>`}
    ).join('');
